...
Monte Carlo eXtreme (MCX) - CUDA Edition - with Gegenbauer phase function
=========================
...
MCX v2024.2 with modifications apported to implement the Gegenbauer phase function.
...
Changes include definition of the Gegenbauer (GEG) phase function in mcx_core.cu, instead of commonly used Henyey-Greenstein phase function. This change allows to have a more accurate simulation of short light propagation distances. The modified phase function embeds more general Gegenbauer polynomials and a second order moment g2 in addition to usual methods that only consider first order moment g1, to tune backward scattering in a more accurate way for not yet isotropic systems. This phase functions provides an analytical solution for sampling values of cos(theta) from random numbers, therefore no numerical method is necessary. 

New variables \gamma and \alpha have been defined and have to be given as input in the form cfg.gamma and cfg.alpha. The variable gamma is defined as gamma = (1-g2)/(1-g1). It is a similarity relation so as \mu'_s, meaning that all combinations of g1 and g2 that lead to the same gamma, also lead to the same reflectance, and is therefore convenient in reducing the number of parameters. Limits for gamma require: cfg.gamma <= 3
The variable alpha is an additional anisotropy parameter required by the phase functions, which needs to be >-0.5

Due to the complex form of its moments g1 and g2, a lookup table is needed to retrieve the parameters of the Gegenbauer phase function from the input values of gamma and alpha. Some code files have been added in the folder for this purpose, namely:

* gamma_selection.m	-> actual function to call to retrieve alpha_Geg and g_Geg
* gammaGeg.mat		-> table containing all values of gamma from entries of alpha_Geg and g_Geg (converted to g1 and g2)
* aGeg.mat		-> table with all entries of alpha_Geg
* gGeg.mat		-> table with all entries of g_Geg
* GEG_g1.m		-> function to retrieve g1 from alpha and g
* GEG_g2.m		-> function to retrieve g2 from alpha and g

STILL TO DO: 	the procedure to retrieve the parameters from the input of gamma has to be improved. Consider also alternatives to the lookup table.
		No change has been apported to add gamma and alpha as json variable. If found necessary, these changes have to be made.
